...
regioes = ['Brasil', 'Centro Oeste', 'Norte', 'Sudeste', 'Sul', 'Nordeste']
...
dados_originais = pd.DataFrame.from_dict(response.json())



# Certifique-se de que a coluna 'Data da Compra' existe no DataFrame original
if 'Data da Compra' in dados_originais.columns:
    dados_originais['Data da Compra'] = pd.to_datetime(dados_originais['Data da Compra'], format='%d/%m/%Y')

filtro_vendedores = st.sidebar.multiselect('Vendedores', dados_originais['Vendedor'].unique())

dados = dados_originais.copy()  # Faça uma cópia dos dados originais
...
# Classifique o DataFrame e selecione os top 5 estados
top5_estados = receita_estados.sort_values('Preço', ascending=False).head(5)

fig_receita_estados = px.bar(top5_estados,
                             x='Local da compra',
                             y='Preço',
                             text='Preço',
...
with aba2:
    ...
    opcao_grafico = st.selectbox("Selecione o tipo de gráfico", ["Mapa de Vendas", "Gráfico Mensal", "Top 5 Estados", "Por Categoria"])

    if opcao_grafico == "Mapa de Vendas":
        st.plotly_chart(fig_mapa_vendas, use_container_width=True)
    elif opcao_grafico == "Gráfico Mensal":
        st.plotly_chart(fig_vendas_mensal, use_container_width=True)
    elif opcao_grafico == "Top 5 Estados":
        st.plotly_chart(fig_vendas_estados, use_container_width=True)
    elif opcao_grafico == "Por Categoria":
